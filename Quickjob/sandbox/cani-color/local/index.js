...
function isSymbol(value) {
  return (typeof value === 'undefined' ? 'undefined' : _typeof$4(value)) == 'symbol' || isObjectLike(value) && baseGetTag(value) == symbolTag;
}
...
var hexaToDecimal = function hexaToDecimal(hexaStr) {
	return parseInt(hexaStr.toLowerCase(), 16);
};

var decimalToHexa = function decimalToHexa(num) {
	return num.toString(16).toLowerCase();
};

var isValidPercent = function isValidPercent(percent) {
	return Number.isFinite(percent) && percent >= 0 && percent <= 1;
};

// generic string

function sliceEvery(str, num) {
	var memo = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];

	if (str === "") {
		return memo;
	}
	return sliceEvery(str.slice(num), num, memo.concat([str.slice(0, num)]));
}

// dealing with ranges of numeric values

var getRangeDelta = function getRangeDelta(range) {
	return range.end - range.start;
};

var getRangeStart = function getRangeStart(delta, center) {
	return Math.round(center - delta / 2);
};

var getRangeEnd = function getRangeEnd(delta, center) {
	return Math.round(center + delta / 2);
};
...
function getRangeFraction(range, center, percent) {
	var fractionDelta = getRangeDelta(range) * percent,
...
var isHexaColor = function isHexaColor(color) {
	return (/^#(?:[0-9a-fA-F]{3}){1,2}$/.test(color)
	);
};

var isShorthandHexaColor = function isShorthandHexaColor(color) {
	return (/^#(?:[0-9a-fA-F]{3})$/.test(color)
	);
};

var inflateHexaColor = function inflateHexaColor(color) {
	return isShorthandHexaColor(color) ? "#" + sliceEvery(color.slice(1), 1).map(function (str) {
		return "" + str + str;
	}).join("") : color;
};
...
function RGBToHexaColor(color) {
	return "#" + color.map(function (num) {
		return padStart(decimalToHexa(num), 2, "0");
	}).join("");
}
...
function getRandomColorComponent() {
	var proximity = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
	var percent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
	var start = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
	var end = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 255;

	var range = getRangeFraction({
		start: start,
		end: end
	}, proximity, percent);
...
function validateGetRandomColor() {
	if (!isHexaColor(arguments.length <= 0 ? undefined : arguments[0])) {
		throw new Error("If passed, the first parameter should be a valid hexadecimal RGB color starting with #.");
	}

	if (!isValidPercent(arguments.length <= 1 ? undefined : arguments[1])) {
		throw new Error("If passed, the second parameter should be a number between 0 and 1 inclusive.");
	}
}

function getRandomColor() {
	var proximity = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "#000000";
	var percent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;

	validateGetRandomColor(proximity, percent);

	var proximityRGB = hexaToRGBColor(proximity);

	return RGBToHexaColor([getRandomColorComponent(proximityRGB[0], percent), getRandomColorComponent(proximityRGB[1], percent), getRandomColorComponent(proximityRGB[2], percent)]);
}

// the app itself

function randomCard(options) {
	var backgroundColor = getRandomColor(at(options, "background.proximity")[0], at(options, "background.percent")[0]);

	var foregroundColor = getRandomColor(at(options, "foreground.proximity")[0], at(options, "foreground.percent")[0]);
